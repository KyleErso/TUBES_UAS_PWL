...
class CommitteeController extends Controller
{
    ...
    public function storeEvent(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'waktu'     => 'required|string', // <--- tambahkan validasi waktu
    ...
    /**
     * Proses menonaktifkan event (ubah status event jadi nonaktif)
     */
    public function deactivateEvent($id)
    {
        $this->authorizePanitia();

        $response = \Illuminate\Support\Facades\Http::withToken(\Illuminate\Support\Facades\Session::get('token'))
            ->patch("{$this->apiUrl}/dashboard/event/{$id}/deactivate");
        ...
        return back()
            ->with('error', 'Gagal menonaktifkan event: ' . $response->body());
    }
}
