...
exports.register = async (req, res) => {
    try {
        const { nama, email, password, role, nim, fakultas, jurusan } = req.body;

        if (!nama || !email || !password || !nim) {
            return res.status(400).json({ message: 'Semua field wajib diisi.' });
        }

        if (await User.findOne({ $or: [{ email }, { nim }] })) {
            return res.status(400).json({ message: 'Email atau NIM sudah terdaftar.' });
        }

        const newUser = await User.create({ nama, email, password, role, nim});
